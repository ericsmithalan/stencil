...
import { IBounds, IDegree, ISize, IVector2 } from "@core.interfaces";
...
export abstract class SVGBase<TProps extends ISVGProps, TState extends ISVGState> extends Control<TProps, TState> {
	...
	public render() {
		console.log(this.width, this.state.width);
		return (
			<div ref={this._containerRef} className="svg-container" style={{ width: this.width, height: this.height }}>
				<svg
					...
					className="svg-image"
					fill={this.fill}
					style={{ width: this.width, height: this.height, fillRule: "evenodd" }}
	...
	protected transformString(): string {
		return `rotate(${this.rotate.deg}) translate(${this.translate.x} ${this.translate.y}) scale(${this.scale.x} ${
			this.scale.y
		})`;
	}
}
